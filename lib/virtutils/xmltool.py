...
def build_domain_xml(dom,exclude=[]):
    ...
    xml += libvirttemplate.GENERAL_METADATA(dom.get('domain_type'), dom.get('name'), dom.get('uuid'))
...
def build_device_xml(dev,exclude=[]):
    ...
    if dev.get('device_type') == 'interface':
        xml += libvirttemplate.INTERFACE(dev.get('interface_type'), 
                                         ...
                                         dev.get('script'),
                                         dev.get('model'))
    if dev.get('device_type') == 'emulator':
        xml += libvirttemplate.EMULATOR(dev.get('emulator'))
        
        
    if dev.get('device_type') == 'graphics':
